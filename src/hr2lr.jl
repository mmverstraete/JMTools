...
# Purpose(s):
* Downsize the positional argument `matrix_hr` by the shrink `factor` in each dimension, and return a shrunk matrix `matrix_lr` of the same type as `matrix_hr`. The two dimensions of `matrix_hr` must be multiples of `factor`. The algorithm used to determine the values of the elements in `matrix_lr` depends on the value of the positional argument `innature`.

# Positional argument(s):
* `matrix_hr::AbstractMatrix{<:Real}`: The 2-D real array to be downsized.
...
* In this code, a value is meant to be `usable` if it corresponds to a meaningful geophysical value, by opposition to flags representing missing values; this is established by specifying, for each `innature` type of data, the minimum and maximum valid values.
* If `factor < 2`, an error condition is raised.

the output value `matrix_lr` is identical to positional argument `matrix_hr`; and if `factor == 0` while `typeof(matrix_hr) <: Number`, all elements of `matrix_lr` are set to `0`.

* Since this function averages up to 16 original high spatial resolution values to estimate 1 low resolution value, it reduces the information content of the original file. Contrast this with the function `lr2hr.jl`, which appears to implement the reverse transformation, but only duplicates values. Hence `lr2hr(hr2lr(hr_array))` is generally not equivalent to `hr_array`.
...
function hr2lr(
    ...
    if factor <= 0
        error("hr2lr: The positional argument factor must be a positive integer.")
    end
